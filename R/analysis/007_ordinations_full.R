...
# Loop through zero percentage thresholds
for (zero_threshold in zero_thresholds) {
  cat("Testing", zero_threshold, "% zero threshold:\n")
  
  # First filter by zero percentage
  zero_filtered <- filter_zero_asvs(
    filtered_phyloseq, 
    max_zero_percent = zero_threshold
  )
  ...
  for (sample_reads in min_sample_reads_thresholds) {
    for (asv_reads in min_asv_reads_thresholds) {
      ...
      result_name <- paste0("zero_", zero_threshold, "_sample_", sample_reads, "_asv_", asv_reads)
      
      # Apply phyloseq filtering
      tryCatch({
        highly_filtered <- filter_phyloseq(
          zero_filtered$phyloseq,
        ...
        filtered_results[[result_name]] <- list(
          ...
          dimensions = c(nsamples(highly_filtered), ntaxa(highly_filtered)),
          zero_threshold = zero_threshold,
          sample_threshold = sample_reads,
          asv_threshold = asv_reads
        )
        
        cat("  ", result_name, ":", nsamples(highly_filtered), "samples,", ntaxa(highly_filtered), "ASVs\n")
...
for (name in names(filtered_results)) {
  ...
  # Extract ASV matrix
  filtered_asv_matrices[[name]] <- as.matrix(as.data.frame(otu_table(
    phyloseq_obj,
    taxa_are_rows = TRUE
  )))
}
...
save(distance_matrices_filtered, file = here::here("data/output/distance_matrices_filtered.rda"))
...
for (name in names(distance_matrices_filtered)) {
  ...
  cat("  Matrix size:", matrix_size, "samples, Memory estimate:", round(memory_estimate, 3), "GB\n")
  
  # Only proceed if memory estimate is reasonable
  if (memory_estimate < 10) {
    tryCatch({
    ...
    pcoa_results_filtered[[name]] <- list(error = "Memory estimate too large")
  }
}

save(pcoa_results_filtered, file = here::here("data/output/pcoa_results_filtered.rda"))
...
for (name in names(filtered_results)) {
  if (name %in% names(pcoa_results_filtered) && 
      !("error" %in% names(pcoa_results_filtered[[name]]))) {
    
    # Score based on number of samples and ASVs
    score <- filtered_results[[name]]$dimensions[1] * filtered_results[[name]]$dimensions[2]
    if (score > best_score) {
      best_score <- score
      best_filter <- name
    }
...
if (!is.null(best_filter)) {
  ...
  save(nmds_result_filtered, file = here::here("data/output/nmds_result_filtered.rda"))
...
if ("full_asv_matrix" %in% names(distance_matrices)) {
  ...
  cat("Full matrix size:", matrix_size_full, "samples, Memory estimate:", round(memory_estimate_full, 2), "GB\n")
  
  if (memory_estimate_full < 20) {  # Higher threshold for full analysis
    ...
    cat("Full dataset too large for analysis - using filtered results only\n")
  }
}
