...
path <- "/Users/jaejinlee/Files/Data/2023SABR_amplicon/demultiplxed"
...
filter_stats <- filterAndTrim(fnFs, filtFs, fnRs, filtRs,
  truncLen = c(140, 140),
  maxN = 0,
  maxEE = c(4, 4),
  truncQ = 2,
...
cat("Samples names - below 50% of the mean sequencing depth:\n", low_samples_mean, "\n\n")
cat("#Samples - below 50% of the mean sequencing depth:", length(low_samples_mean), "\n")
...
ggplot(depth_data, aes(x = sequencing_depth)) +
  geom_histogram(aes(y = ..density..), binwidth = 1000, fill = "steelblue", alpha = 0.6) +
  geom_density(color = "darkred", size = 1) +
  geom_vline(xintercept = c(5000, 10000, mean(merger_summary) * 0.5), color = "red", linetype = "dashed", size = 0.8) +
  geom_vline(xintercept = mean(merger_summary), color = "black", linetype = "dashed", size = 0.8) +
  labs(
...
# removing chimeras
seqtab.nochim <- removeBimeraDenovo(seqtab_filtered, method = "consensus", multithread = TRUE, verbose = TRUE)
...
# loading metadata
metadata <- read.csv("/Users/jaejinlee/Files/Data/2023SABR_amplicon/SABR2023_metadata.csv", row.names = 1)
head(metadata)
...
saveRDS(ps, file = "/Users/jaejinlee/Files/Data/2023SABR_amplicon/analysis/ps_object.rds")
...
# save as a csv file
write.csv(asv_data, file = "/Users/jaejinlee/Files/Data/2023SABR_amplicon/analysis/asv_table.csv", row.names = TRUE)
...
# save as a csv file
write.csv(asv_table_rel_df, file = "/Users/jaejinlee/Files/Data/2023SABR_amplicon/analysis/asv_table_ps_rel.csv", row.names = TRUE)
...
asv_count_df <- data.frame(OTU = names(asv_sample_counts), Sample_Counts = asv_sample_counts)
...
nmds_high_prev <- ordinate(ps_high_prev, method = "NMDS", distance = "bray", trymax = 100)

nmds_plot_high_prev <- plot_ordination(ps_high_prev, nmds_high_prev, color = "Plant", shape = "Location") +
  geom_point(size = 3, alpha = 0.8) +
...
low_prevalence_asvs_u70 <- names(asv_sample_counts[asv_sample_counts < threshold_u70])
...
nmds_low_prev_u70 <- ordinate(ps_low_prev_u70, method = "NMDS", distance = "bray", trymax = 100)

# visualization
nmds_plot_low_prev_u70 <- plot_ordination(ps_low_prev_u70, nmds_low_prev_u70, color = "Plant", shape = "Location") +
  geom_point(size = 3, alpha = 0.8) +
  facet_wrap(~Date, ncol = 2) +
  labs(
...
low_prevalence_asvs_u30 <- names(asv_sample_counts[asv_sample_counts < threshold_u30])
...
nmds_low_prev_u30 <- ordinate(ps_low_prev_u30, method = "NMDS", distance = "bray", trymax = 100)

# visualization
nmds_plot_low_prev_u30 <- plot_ordination(ps_low_prev_u30, nmds_low_prev_u30, color = "Plant", shape = "Location") +
  geom_point(size = 3, alpha = 0.8) +
  facet_wrap(~Date, ncol = 2) +
  labs(
...
core_asvs_P01_I <- names(asv_sample_counts_P01_I[asv_sample_counts_P01_I >= threshold_80_P01_I])
...
core_asvs_P01_P08_I <- names(asv_sample_counts_P01_P08_I[asv_sample_counts_P01_P08_I >= threshold_80_P01_P08_I])

cat("ASVs found in >80% P01_I + P08_I samples:", length(core_asvs_P01_P08_I), "\n")

# NMDS for P01-I and P08-I
nmds_P01_P08_I <- ordinate(ps_corn_P01_P08_I, method = "NMDS", distance = "bray", trymax = 100)

nmds_plot_P01_P08_I <- plot_ordination(ps_corn_P01_P08_I, nmds_P01_P08_I, color = "Plot", shape = "Date") +
  geom_point(size = 3, alpha = 0.8) +
  labs(
...
## P01_I, P08_I, P11_I
ps_corn_P01_P08_P11_I <- merge_phyloseq(ps_corn_P01_I, ps_corn_P08_I, ps_corn_P11_I)
...
core_asvs_P01_P08_P11_I <- names(asv_sample_counts_P01_P08_P11_I[asv_sample_counts_P01_P08_P11_I >= threshold_80_P01_P08_P11_I])

cat("ASVs found in >80% P01_I + P08_I + P11_I samples:", length(core_asvs_P01_P08_P11_I), "\n")

# NMDS for P01_I, P08_I, P11_I
nmds_P01_P08_P11_I <- ordinate(ps_corn_P01_P08_P11_I, method = "NMDS", distance = "bray", trymax = 100)

nmds_plot_P01_P08_P11_I <- plot_ordination(ps_corn_P01_P08_P11_I, nmds_P01_P08_P11_I, color = "Plot", shape = "Date") +
  geom_point(size = 3, alpha = 0.8) +
  labs(
...
ps_corn_P01_P08_P11_P14_I <- merge_phyloseq(ps_corn_P01_I, ps_corn_P08_I, ps_corn_P11_I, ps_corn_P14_I)

asv_table_data_P01_P08_P11_P14_I <- as.data.frame(otu_table(ps_corn_P01_P08_P11_P14_I))
total_samples_P01_P08_P11_P14_I <- nrow(asv_table_data_P01_P08_P11_P14_I)

asv_sample_counts_P01_P08_P11_P14_I <- colSums(asv_table_data_P01_P08_P11_P14_I > 0)

threshold_80_P01_P08_P11_P14_I <- total_samples_P01_P08_P11_P14_I * 0.8
core_asvs_P01_P08_P11_P14_I <- names(asv_sample_counts_P01_P08_P11_P14_I[asv_sample_counts_P01_P08_P11_P14_I >= threshold_80_P01_P08_P11_P14_I])

cat("ASVs found in >80% P01_I + P08_I + P11_I + P14_I samples:", length(core_asvs_P01_P08_P11_P14_I), "\n")

# NMDS
nmds_P01_P08_P11_P14_I <- ordinate(ps_corn_P01_P08_P11_P14_I, method = "NMDS", distance = "bray", trymax = 100)

nmds_plot_P01_P08_P11_P14_I <- plot_ordination(ps_corn_P01_P08_P11_P14_I, nmds_P01_P08_P11_P14_I, color = "Plot", shape = "Date") +
  geom_point(size = 3, alpha = 0.8) +
  labs(
...
## Corn_Interrow + P01_R
ps_corn_I_P01_R <- merge_phyloseq(ps_corn_P01_I, ps_corn_P08_I, ps_corn_P11_I, ps_corn_P14_I, ps_corn_P01_R)
...
core_asvs_corn_I_P01_R <- names(asv_sample_counts_corn_I_P01_R[asv_sample_counts_corn_I_P01_R >= threshold_80_corn_I_P01_R])

cat("ASVs found in >80% corn_I+P01_RI", length(core_asvs_corn_I_P01_R), "\n")

# NMDS
nmds_corn_I_P01_R <- ordinate(ps_corn_I_P01_R, method = "NMDS", distance = "bray", trymax = 100)

nmds_plot_corn_I_P01_R <- plot_ordination(ps_corn_I_P01_R, nmds_corn_I_P01_R, color = "Plot", shape = "Date") +
  geom_point(size = 3, alpha = 0.8) +
  labs(
...
ps_corn_I_P01_P08_R <- merge_phyloseq(ps_corn_P01_I, ps_corn_P08_I, ps_corn_P11_I, ps_corn_P14_I, ps_corn_P01_R, ps_corn_P08_R)
...
asv_sample_counts_corn_I_P01_P08_R <- colSums(asv_table_data_corn_I_P01_P08_R > 0)

threshold_80_corn_I_P01_P08_R <- total_samples_corn_I_P01_P08_R * 0.8
core_asvs_corn_I_P01_P08_R <- names(asv_sample_counts_corn_I_P01_P08_R[asv_sample_counts_corn_I_P01_P08_R >= threshold_80_corn_I_P01_P08_R])

cat("ASVs found in >80% corn_Interrow + P01_R + P08_R samples:", length(core_asvs_corn_I_P01_P08_R), "\n")

# NMDS
nmds_corn_I_P01_R_P08_R <- ordinate(ps_corn_I_P01_P08_R, method = "NMDS", distance = "bray", trymax = 100)

nmds_plot_corn_I_P01_R_P08_R <- plot_ordination(ps_corn_I_P01_P08_R, nmds_corn_I_P01_R_P08_R, color = "Plot", shape = "Date") +
  geom_point(size = 3, alpha = 0.8) +
  labs(
...
ps_corn_I_P01_P08_P11_R <- merge_phyloseq(ps_corn_P01_I, ps_corn_P08_I, ps_corn_P11_I, ps_corn_P14_I, ps_corn_P01_R, ps_corn_P08_R, ps_corn_P11_R)

asv_table_data_corn_I_P01_P08_P11_R <- as.data.frame(otu_table(ps_corn_I_P01_P08_P11_R))

total_samples_corn_I_P01_P08_P11_R <- nrow(asv_table_data_corn_I_P01_P08_P11_R)

asv_sample_counts_corn_I_P01_P08_P11_R <- colSums(asv_table_data_corn_I_P01_P08_P11_R > 0)

threshold_80_corn_I_P01_P08_P11_R <- total_samples_corn_I_P01_P08_P11_R * 0.8
core_asvs_corn_I_P01_P08_P11_R <- names(asv_sample_counts_corn_I_P01_P08_P11_R[asv_sample_counts_corn_I_P01_P08_P11_R >= threshold_80_corn_I_P01_P08_P11_R])

cat("ASVs found in >80% corn_I+P01_P08_P11_R samples:", length(core_asvs_corn_I_P01_P08_P11_R), "\n")

# NMDS
nmds_corn_I_P01_R_P08_R_P11_R <- ordinate(ps_corn_I_P01_P08_P11_R, method = "NMDS", distance = "bray", trymax = 100)

nmds_plot_corn_I_P01_R_P08_R_P11_R <- plot_ordination(ps_corn_I_P01_P08_P11_R, nmds_corn_I_P01_R_P08_R_P11_R, color = "Plot", shape = "Date") +
  geom_point(size = 3, alpha = 0.8) +
  labs(
...
ps_corn_I_P01_P08_P11_P14_R <- merge_phyloseq(ps_corn_P01_I, ps_corn_P08_I, ps_corn_P11_I, ps_corn_P14_I, ps_corn_P01_R, ps_corn_P08_R, ps_corn_P11_R, ps_corn_P14_R)

asv_table_data_corn_I_P01_P08_P11_P14_R <- as.data.frame(otu_table(ps_corn_I_P01_P08_P11_P14_R))

total_samples_corn_I_P01_P08_P11_P14_R <- nrow(asv_table_data_corn_I_P01_P08_P11_P14_R)

asv_sample_counts_corn_I_P01_P08_P11_P14_R <- colSums(asv_table_data_corn_I_P01_P08_P11_P14_R > 0)

threshold_80_corn_I_P01_P08_P11_P14_R <- total_samples_corn_I_P01_P08_P11_P14_R * 0.8
core_asvs_corn_I_P01_P08_P11_P14_R <- names(asv_sample_counts_corn_I_P01_P08_P11_P14_R[asv_sample_counts_corn_I_P01_P08_P11_P14_R >= threshold_80_corn_I_P01_P08_P11_P14_R])

cat("ASVs found in >80% corn_I+R:", length(core_asvs_corn_I_P01_P08_P11_P14_R), "\n")

# NMDS
nmds_corn_I_R <- ordinate(ps_corn_I_P01_P08_P11_P14_R, method = "NMDS", distance = "bray", trymax = 100)

nmds_plot_corn_I_R <- plot_ordination(ps_corn_I_P01_P08_P11_P14_R, nmds_corn_I_R, color = "Plot", shape = "Date") +
  geom_point(size = 3, alpha = 0.8) +
  labs(
...
threshold_80_corn_I_P01_P08_P11_P14_R <- total_samples_corn_I_P01_P08_P11_P14_R * 0.7
core_asvs_corn_I_P01_P08_P11_P14_R <- names(asv_sample_counts_corn_I_P01_P08_P11_P14_R[asv_sample_counts_corn_I_P01_P08_P11_P14_R >= threshold_80_corn_I_P01_P08_P11_P14_R])
cat("ASVs found in >80% corn_I+R:", length(core_asvs_corn_I_P01_P08_P11_P14_R), "\n")
...
write.csv(asv_table_data, file = "/Users/jaejinlee/Files/Data/2023SABR_amplicon/analysis/asv_table_ps_rel_all.csv", row.names = TRUE)
...
ggplot(prevalence_abundance_df, aes(x = Prevalence, y = Abundance)) +
  ...
  geom_point(
    ...
    aes(x = Prevalence, y = Abundance), color = "blue", alpha = 0.8
  ) + # dots in blue
...
blue_asvs <- prevalence_abundance_df$OTU[prevalence_abundance_df$Prevalence >= 90]
...
# save as a CSV file
write.csv(blue_asvs_df, file = "/Users/jaejinlee/Files/Data/2023SABR_amplicon/analysis/blue_asvs_table.csv", row.names = FALSE)
...
ggplot(prevalence_abundance_df_1, aes(x = Prevalence, y = Abundance)) +
  ...
  geom_point(
    data = prevalence_abundance_df_1[prevalence_abundance_df_1$Prevalence >= 80 &
      prevalence_abundance_df_1$Abundance >= abundance_threshold_1, ],
    aes(x = Prevalence, y = Abundance), color = "blue", alpha = 0.8
  ) +
  scale_y_log10(labels = scales::trans_format("log10", scales::math_format(.x))) + # log10 value
  geom_vline(xintercept = 80, linetype = "dashed", color = "red", size = 1) +
  geom_hline(yintercept = abundance_threshold_1, linetype = "dashed", color = "green", size = 1) +
  labs(
...
selected_otus_1 <- prevalence_abundance_df_1$OTU[
  prevalence_abundance_df_1$Prevalence >= 80 & prevalence_abundance_df_1$Abundance >= abundance_threshold_1
]

# Count the number of selected OTUs
cat("OTUs meeting both criteria (Prevalence ≥ 80% & Top 20% Abundance):", length(selected_otus_1), "\n")
...
write.csv(selected_otus_df_1, file = "/Users/jaejinlee/Files/Data/2023SABR_amplicon/selected_otus_table_1.csv", row.names = FALSE)
...
cat("P01+P08 → P01+P08+P11 단계에서 제거된 ASV 개수:", length(removed_P11), "\n")
cat("P01+P08+P11 → P01+P08+P11+P14 단계에서 제거된 ASV 개수:", length(removed_P14), "\n")
...
cat("P01+P08+P11 → P01+P08+P11+P14 단계에서 추가된 ASV 개수:", length(added_P14), "\n")
...
asv_matrix <- data.frame(
  ...
  P14 = as.integer(unique(unlist(core_asvs_list)) %in% core_asvs_P01_P08_P11_P14_I)
)
asv_matrix$P14
...
asv_matrix_long <- asv_matrix %>%
  ...
  pivot_longer(cols = -c(ASV, ASV_new), names_to = "Sample", values_to = "Presence") %>%
...
ggplot(asv_matrix_long, aes(x = Sample, y = ASV_new, fill = factor(Presence))) +
  ...
  theme(
    ...
    axis.text.y = element_text(size = 8, hjust = 1, vjust = 0.5, margin = margin(r = -20)), # Y축 레이블과 그래프 사이 간격 줄이기
  ...
  labs(title = "Core ASV (existing >80% samples)", x = "Corn-Interrow Plots", y = "ASV")
