...
filtered_phyloseq <- subset_samples(filtered_phyloseq, brc == params$BRC)
...
bray_curtis_curve <- gg_bc_curve(core_summary_list = core_summary_lists)
occ_abun_plot <- gg_occurance_curve(core_summary_list = core_summary_lists)

ggsave(
  filename = str_glue("{params$BRC}_bray_curtis_curve.png"),
  bray_curtis_curve,
  path = str_glue("data/output/plots/{params$BRC}/"),
  dpi = 300,
  width = 6,
  height = 4
)
...
save(
  core_asvs_threshold,
  file = "data/output/phyloseq_objects/{params$BRC}/{params$BRC}_high_threshold.rda"
)
```